...
from work_in_progress import dictionary
...
class Vowel:
    ...
    def __init__(self, vowel, follow):
        ...
        self.longitude = decide_on_length(self.vowel, follow)
...
def main(path_to_text, path_to_result, all=True, sectionSize=20, trace=True):
    ...
    for key, value in roots.items():
        empty = 0
        for end in value.endings:
            if end[3] is not None:
                if len(scansion_versions(lines[end[1]].get_meter(), METER, 0)) == 0:
                    empty += 1
        if empty > 1:
            smalldict[key] = empty
    ordered = sorted(smalldict, key=smalldict.get, reverse=True)
    for i in ordered:
        print(i + ' ' + str(smalldict[i]))"""
...
if __name__ == "__main__":
    ...
    # main('texts/Seneca.txt', 'output/Seneca.txt')
    # main('texts/Thyestes.txt', 'output/Thyestes.txt')
